// creates folder structure for new Microservice following Clean Architecture principles.

package main

import (
	"flag" // used for parsing command-line arguments (e.g., -name user).
	"fmt"
	"os"
	"path/filepath"
)
...
func main() {
	// defines a command line flag "-name" --> which takes the name of the service
	...
	// Create service directory structure --> representing Clean Architecture
	...
	for _, dir := range dirs {
		...
		if err := os.MkdirAll(fullPath, 0755); err != nil { // 0755 --> sets the directory permission as owner.
